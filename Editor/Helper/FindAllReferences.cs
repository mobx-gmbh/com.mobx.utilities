    ...
    public class FindAllReferences : UnityEditor.EditorWindow
    {
        ...
        private static void FindInProject()
        {
            ...
            if (UnityEditor.PrefabUtility.GetPrefabAssetType(selectedObj) != UnityEditor.PrefabAssetType.NotAPrefab)
            {
                ...
                {
                    selectedObj = UnityEditor.AssetDatabase.LoadMainAssetAtPath(UnityEditor.AssetDatabase.GetAssetPath(selectedObj.GetInstanceID()));
                }
            }
        ...
        private static void CheckScene(Scene scene, bool inHierarchy, Object asset = null)
        {
            ...
            foreach (var go in allObjects)
            {
                if (isFindingGameObject)
                {
                    UnityEditor.EditorUtility.DisplayProgressBar("Hierarchy", "Searching...", 1f * ++progress / totalCount);
                }
        ...
        private static void CheckReference(Object obj, bool inHierarchy, Object asset = null)
        {
            ...
            do
            {
                if (p.propertyType != UnityEditor.SerializedPropertyType.ObjectReference || p.objectReferenceValue == null ||
                ...
                else if (isFindingAsset)
                {
                    if (p.objectReferenceValue == selectedObj ||
                        selectedIsMainAsset && UnityEditor.AssetDatabase.GetAssetPath(p.objectReferenceValue) == selectedObjPath)
                    {
                        isMatched = true;
                    }
                }
        ...
        private void Init()
        {
            _hierarchyLabelIcon = new GUIContent(" Hierarchy", UnityEditor.EditorGUIUtility.FindTexture("UnityEditor.HierarchyWindow"));
        ...
        private void OnResultsInHierarchyGUI()
        {
            ...
            //header
            GUILayout.Space(-15);
            UnityEditor.EditorGUILayout.LabelField(_hierarchyLabelIcon, _headerStyle, GUILayout.Width(200), GUILayout.Height(20));
            ...
            foreach (var pair in hierarchyResults)
            {
                for (var i = 0; i < pair.Value.Count; i++)
                {
                    ...
                    {
                        UnityEditor.EditorGUI.LabelField(rect, new GUIContent(UnityEditor.AssetPreview.GetMiniThumbnail(r.Component)), _labelStyle);
                        rect.x += 25;
                        rect.width -= 25;
        ...
        private void OnResultsInProjectGUI()
        {
            ...
            //header
            GUILayout.Space(-15);
            UnityEditor.EditorGUILayout.LabelField(_projectLabelIcon, _headerStyle, GUILayout.Width(200), GUILayout.Height(20));
